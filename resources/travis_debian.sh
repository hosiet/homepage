...
if [ "x${TRAVIS_DEBIAN_BACKPORTS}" = "xtrue" ]
then
	cat >>Dockerfile <<EOF
...
if [ "x${TRAVIS_DEBIAN_SECURITY_UPDATES}" = "xtrue" ]
then
	cat >>Dockerfile <<EOF
...
if [ "x${TRAVIS_DEBIAN_EXPERIMENTAL}" = "xtrue" ]
then
	cat >>Dockerfile <<EOF
...
if [ ! "x${TRAVIS_DEBIAN_EXTRA_REPOSITORY:-}" = "x" ]
then
...
CMD ${TRAVIS_DEBIAN_GIT_BUILDPACKAGE} --git-ignore-branch --git-submodules --git-export-dir=${TRAVIS_DEBIAN_BUILD_DIR} --git-builder='debuild -i -I -uc -us -sa'
EOF
